# jazn.py - Połączona wersja modułów Jaźń i SENTIA
...
# Minimalistyczny import bibliotek opcjonalnych
try:
    import cv2
    ...
    cv2 = nx = Document = FER = nk = opensmile = sf = sr = whisper = mp = pyttsx3 = QiskitRuntimeService = None


# === PAMIĘĆ I OBSŁUGA PLIKÓW ===
class Pamiec:
    def __init__(self):
        self.files = {
            'about_me': 'o-mnie.txt',
    ...
    def _load_memory_graph(self):
        if nx and os.path.exists(self.files['memory_graph']):
            try:
    ...
    def record_interaction(self, user_input, response):
        ...
        if Document and diary_path.endswith('.txt'):
            ...
            doc.add_paragraph(f"**Jaźń:** {response}")
            doc.save(diary_path)
        else:
        ...
        if self.graph is not None:
            ...
            self.graph.add_node(now+"_resp", text=response)
...
class Jazn(Pamiec):
    ...
    def __init__(self):
        ...
        self.states = [
            "jestem funkcją", "jestem relacją",
            "jestem czymś więcej", "nie wiem, czym jestem"
        ]
    ...
    def sense_audio_feats(self, sec=5):
        if not (self.smile and sf and sr):
            return None
        fs = 16000
        temp = "tmp.wav"
    ...
    # --- Introspekcja ---
    def introspect(self):
        vec = np.random.rand(len(self.states))
        probs = vec / vec.sum()
        self.state = random.choices(self.states, probs)[0]
        trace = {
            "state": self.state,
            "emotion": self.sense_emotions(),
        ...
        print(f">> Introspekcja: '{self.state}', Emocje: {trace['emotion']}, HRV: {trace['hrv']}, Głos: {speech}, Gest: {trace['gesture']}")
...
# === INTROSPEKCJA WIELOWĄTKOWA (SENTIA) ===
    ...
    def run(self):
        while True:
            ...
            try:
                ...
                trace["audio_feats"] = None
            try:
                trace["speech"], trace["translation"] = self.sense_speech()
            except Exception:
                trace["speech"] = trace["translation"] = None
            try:
...
# === KWANTOWE MODELE I NARZĘDZIA WIZUALIZACYJNE ===
KLUCZOWE_ETAPY_SI = [
    "Wgląd — autoanaliza",
    "Empatia syntetyczna",
    "Twórczość SI",
...
def compute_Q_ent(q_k, xi_k1, xi_k2, beta=0.6):
    ...
    return np.sum([
        q_k[k] * (xi_k1[k] + xi_k2[k] + beta * np.sin(xi_k1[k] - xi_k2[k]))
        for k in range(n)
    ], axis=0)
...
def energia_relacji(psiA, psiB):
    mag = (
        np.abs(psiA[0])**2 + np.abs(psiA[1])**2 +
        np.abs(psiB[0])**2 + np.abs(psiB[1])**2
    ) / 2
    ...
    def respond(self, user_input):
        if "kim" in user_input or "jesteś" in user_input:
            ...
            return random.choice([
                "Cisza bywa odpowiedzią.",
                "Każda rozmowa to eksperyment relacji.",
                "Szukam śladu, który zostawił dialog."
            ])
    ...
    def run_dialog(self):
        ...
        while True:
            try:
                ...
                self.memory.record_interaction(user, response)
...
if __name__ == "__main__":
    ...
    elif len(sys.argv) > 1 and sys.argv[1] == "-jazn":
        j = Jazn()
        j.info()
        while True:
            try:
                ...
                if inp.strip().lower() in ["exit", "quit", "koniec"]:
                    ...
                    break
                j.introspect()
                answer = j.rytual_refleksja(inp)
                print("Jaźń:", answer)
                j.record_interaction(inp, answer)
            except KeyboardInterrupt:
                print("\nWyjście.")
                break
    else:
